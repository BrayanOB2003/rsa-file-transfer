...
public class Server {
    ...
    public static void main(String[] args) {
        ...
        try (ServerSocket serverSocket = new ServerSocket(SERVER_PORT)) { // Use try-with-resources
            System.out.println("Servidor escuchando en el puerto " + SERVER_PORT + " ...");

            while (true) {  // Main loop to accept multiple connections
                try (Socket socket = serverSocket.accept()) { // Try-with-resources for socket
                    ...
                    if (MessageDigest.isEqual(clientHash, serverHash)) {
                        ...
                        System.out.println("Error: El archivo transferido esta corrupto.");
                            ...
                            System.out.println("Transferencia completada. Esperando nueva conexion...");
                        } catch (Exception e) {
                            // Handle individual connection errors
                            System.err.println("Error al manejar la conexion: " + e.getMessage());
                            e.printStackTrace(); 
                        }
                    }
                } catch (IOException e) {
                    // Handle errors with the server socket
